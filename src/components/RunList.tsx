...
type SortColumn = "sequencerGasPerSecond" | "validatorGasPerSecond";

type SortDirection = "asc" | "desc" | "disabled";

// Column definitions with tooltips
const COLUMN_DEFINITIONS = {
  sequencerGasPerSecond: {
    label: "Seq. Gas/s",
    tooltip:
      "Canonical gas produced per second at the configured block cadence",
  },
  validatorGasPerSecond: {
    label: "Val. Gas/s",
    tooltip:
      "Total canonical gas divided by total validator new-payload latency",
  },
} as const;
...
const getMetricValue = (
  ...
  switch (column) {
    case "sequencerGasPerSecond":
      return run.result?.sequencerMetrics?.gasPerSecond ?? 0;
    case "validatorGasPerSecond":
...
const RunList = ({
  ...
  const renderMetricCell = (
    ...
    return <MetricValue value={value} unit="gas/s" />;
  };
  ...
  return (
    <div className="p-6 overflow-x-auto flex-grow border-slate-200">
      {groupedSections.map((section) => {
        ...
        const showSectionHeader = section.key !== "all-runs";
                  ...
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      {Object.entries(COLUMN_DEFINITIONS).map(([key, def]) => (
                        <th
                          key={key}
                          className="px-6 py-3 text-left text-sm font-medium text-slate-700 tracking-wider cursor-pointer transition-colors duration-150 uppercase hover:bg-slate-100"
