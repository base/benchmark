...
const formatWithPrefix = (
  ...
  for (const [prefix, multiplier] of sortedPrefixes) {
    if (Math.abs(value) >= multiplier) {
      const formatted = (value / multiplier).toFixed(decimalPlaces);
      return `${trimTrailingZero ? formatted.replace(/\.0$/, "") : formatted} ${prefix}${baseUnit}`;
    }
  }
  // Should not happen if "" prefix with value 1 is included, but as fallback:
  const formatted = value.toFixed(decimalPlaces);
  return `${trimTrailingZero ? formatted.replace(/\.0$/, "") : formatted} ${baseUnit}`;
};
