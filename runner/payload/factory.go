...
func NewPayloadWorker(ctx context.Context, log log.Logger, testConfig *benchtypes.TestConfig, sequencerClient clienttypes.ExecutionClient, definition Definition) (worker.Worker, error) {
	...
	case "load-test":
		...
		worker, err = loadtest.NewLoadTestPayloadWorker(
			log, sequencerClient.ClientURL(), sequencerClient.FlashblocksWsURL(), params, privateKey, amount, config, genesis.Config.ChainID, *def, testConfig.LoadTestOutputPath)
	case "transfer-only":
