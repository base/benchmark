...
interface BaseChartProps {
  ...
  title?: string
  description?: string
  children: (svg: d3.Selection<SVGGElement, unknown, null, undefined>, dimensions: { width: number; height: number; margin: { top: number; right: number; bottom: number; left: number } }) => void
}
  ...
  useEffect(() => {
    const updateDimensions = () => {
      if (wrapperRef.current) {
        ...
        const calculatedWidth = Math.max(100, width - DEFAULT_MARGIN.left - DEFAULT_MARGIN.right)
        const calculatedHeight = Math.max(50, height - DEFAULT_MARGIN.top - DEFAULT_MARGIN.bottom)
        setDimensions({ width: calculatedWidth, height: calculatedHeight, margin: DEFAULT_MARGIN })
      }
    }

    // Ensure initial dimensions are set even if ResizeObserver fires quickly
    if (!dimensions && wrapperRef.current?.offsetWidth) {
      updateDimensions()
    }
