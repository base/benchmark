...
import (
	...
	"github.com/base/base-bench/server/internal/config"
	"github.com/base/base-bench/server/internal/handlers"
...
func setupRoutes(router *gin.Engine, s3Service *services.S3Service, l log.Logger) {
	...
	// Static file emulation routes - serve files at the same paths as static mode
	// This allows the frontend to use the same URL structure for both static and API modes
	router.GET("/output/metadata.json", handlers.MetadataHandler(s3Service, l))
	router.GET("/output/:outputDir/:filename", handlers.StaticEmulationHandler(s3Service, l))

	api.GET("/load-tests/:network", handlers.LoadTestListHandler(s3Service, l))
	api.GET("/load-tests/:network/:timestamp", handlers.LoadTestResultHandler(s3Service, l))
}
